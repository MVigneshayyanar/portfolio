...
import { ref, onValue, set, get, onDisconnect, serverTimestamp, runTransaction } from 'firebase/database';
...
const useViewCounter = () => {
  ...
  useEffect(() => {
    ...
    if (isConfigured && database) {
      // ✅ FIREBASE MODE: TRUE GLOBAL REAL-TIME TRACKING
      console.log('🌍 Global real-time tracking active! Everyone sees the same count.');
      
      const viewsRef = ref(database, 'portfolio/totalViews');
      const onlineUsersRef = ref(database, 'portfolio/onlineUsers');
      const userRef = ref(database, `portfolio/onlineUsers/${sessionIdRef.current}`);
      ...
      const setupFirebase = async () => {
        try {
          ...
          // Listen to total views in real-time
          onValue(viewsRef, (snapshot) => {
            const views = snapshot.val() || 0;
            setTotalViews(views);
          });

          // Listen to online users count in real-time
          onValue(onlineUsersRef, (snapshot) => {
            const users = snapshot.val() || {};
            const onlineCount = Object.keys(users).length;
            setLiveViewers(onlineCount);
          });

          // Increment total views on first visit
      ...
      // ⚠️ FALLBACK MODE: Local tracking only (same device)
      console.log('⚠️ Local tracking mode. Add Firebase config for global tracking.');
